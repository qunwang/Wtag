...
parser.add_option('-a', '--additioninformation',action="store",type="string",dest="additioninformation",default="EXO_25ns_045")
...
parser.add_option('--fitwtaggersim', action='store_true', dest='fitwtaggersim', default=True, help='fit wtagger jet in ttbar control sample with mu and el samples simultaneously')
...
class doFit_wj_and_wlvj:
    ...
    def __init__(self, in_channel,in_signal_sample, in_mj_min=40, in_mj_max=130, label="", input_workspace=None):
        ...
        #self.file_Directory="TTBarDataSet_PHYS14/";
        #self.file_Directory="TTBarDataSet/";
        self.file_Directory="TTBarDataSet/backup045/";
        ...
        self.file_data = (self.channel+"_PKU_pdata.root");#keep blind!!!!
        self.file_pseudodata = (self.channel+"_PKU_allBkg_xww.root");#fake data
        self.file_signal     = (self.channel+"_PKU_%s_xww.root"%(self.signal_sample));
        self.file_WJets0_mc  = (self.channel+"_PKU_WJetsPt180_xww.root");
        self.file_VV_mc      = (self.channel+"_PKU_VV_xww.root");# WW+WZ
        self.file_TTbar_mc   = (self.channel+"_PKU_TTBARpowheg_xww.root");
        self.file_STop_mc    = (self.channel+"_PKU_SingleTop_xww.root");
        ...
        if self.channel=="el":
            self.pfMET_cut= 40; self.lpt_cut = 50;
   ...
   #### Method to make a RooAbsPdf giving label, model name, spectrum, if it is mc or not and a constraint list for the parameters
    ...
    def make_Pdf(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[]):
        ...
        if in_model_name == "ErfExp_2" :
            ...
            rrv_width_ErfExp  = RooRealVar("rrv_width_ErfExp"+label+"_"+self.channel,"rrv_width_ErfExp"+label+"_"+self.channel,40.,20, 70.);
            model_pdf = ROOT.RooErfExpPdf("model_pdf"+label+"_"+self.channel+mass_spectrum,"model_pdf"+label+"_"+self.channel+mass_spectrum,rrv_x,rrv_c_ErfExp,rrv_offset_ErfExp,rrv_width_ErfExp);
  ...
  #,p0_tmp-p0_tmp_err*4,p0_tmp+p0_tmp_err*4);
        ...
        if in_model_name == "ErfExp_ttbar_failtau2tau1cut":
            ...
            #offset_tmp = 9.98574e+01; offset_tmp_err = 3.40396e-01;
            width_tmp = 3.90e+01 ; width_tmp_err = 1.92162e+00;
            #width_tmp = 5.84289e+01 ; width_tmp_err = 1.92162e+00;
 ...
 ## Define the Extended Pdf for and mJ fit giving: label, fit model name, list constraint and ismc
    ...
    def make_Model_for_ttbar_controlsample(self, label, in_model_name, mass_spectrum="_mj", ConstraintsList=[],info=""):
        ...
        if TString(label).Contains("_ttbar_data") and not TString(label).Contains("failtau2tau1cut"):
            ...
            eff_ttbar = RooRealVar("eff_ttbar_data"+info+"_"+self.channel,"eff_ttbar_data"+info+"_"+self.channel,0.7,0.3,0.99);
        ...
        ## fail fit has to be done after the pass one
            ...
            eff_ttbar = RooRealVar("eff_ttbar_TotalMC"+info+"_"+self.channel,"eff_ttbar_TotalMC"+info+"_"+self.channel,0.7,0.3,0.99);
...
## all the parameters contrained within the assigned error
	...
	if self.wtagger_label=="HP":
            ...
            model_ttbar_data = self.make_Model_for_ttbar_controlsample("_ttbar_data"+label,"2Gaus_ttbar_a","_mj",self.constrainslist_data,label); ## No constraint
            #model_ttbar_data = self.make_Model_for_ttbar_controlsample("_ttbar_data"+label,"GausChebychev_ttbar_failtau2tau1cut","_mj",self.constrainslist_data,label); ## No constraint
	...
	if self.wtagger_label=="HP":
            ...
            model_ttbar_data_fail = self.make_Model_for_ttbar_controlsample("_ttbar_data"+label+"_"+"failtau2tau1cut","2Gaus_ttbar","_mj",self.constrainslist_data,label); 
## No Constraint
	...
	if self.wtagger_label=="HP":
            ...
            model_ttbar_TotalMC = self.make_Model_for_ttbar_controlsample("_ttbar_TotalMC"+label,"2Gaus_ttbar_a","_mj",self.constrainslist_TotalMC,label);
            #model_ttbar_TotalMC = self.make_Model_for_ttbar_controlsample("_ttbar_TotalMC"+label,"GausChebychev_ttbar_failtau2tau1cut","_mj",self.constrainslist_TotalMC,label);
        print " ############################### Fail: MC ttbar ################################### "
	...
	if self.wtagger_label=="HP":
            ...
            model_ttbar_TotalMC_fail = self.make_Model_for_ttbar_controlsample("_ttbar_TotalMC"+label+"_"+"failtau2tau1cut","2Gaus_ttbar","_mj",self.constrainslist_TotalMC,label);
...
#         simPdf_TotalMC.plotOn(xframe_data,RooFit.Name("MC fit"),RooFit.Slice(category_p_f,"pass"),RooFit.ProjWData(RooArgSet(category_p_f),combData_p_f_TotalMC),RooFit.NormRange("controlsample_fitting_range"), RooFit.LineStyle(kDashed))
        ...
        ## plot data fit function        combData_p_f_data.plotOn(xframe_data,RooFit.Name("data invisi"), RooFit.Cut("category_p_f%s_%s==category_p_f%s_%s::pass"%(label,self.channel,label,self.channel)), RooFit.MarkerSize(1.5), RooFit.DataError(RooAbsData.SumW2), RooFit.XErrorSize(0));
        combData_p_f_data.plotOn(xframe_data,RooFit.Name("data invisi"), RooFit.Cut("category_p_f%s_%s==category_p_f%s_%s::pass"%(label,self.channel,label,self.channel)), RooFit.MarkerSize(1.5), RooFit.DataError(RooAbsData.SumW2), RooFit.XErrorSize(0));
# #6
...
#         simPdf_data.plotOn(xframe_data,RooFit.Name("data fit invisi"),RooFit.Slice(category_p_f,"pass"),RooFit.ProjWData(RooArgSet(category_p_f),combData_p_f_data),RooFit.NormRange("controlsample_fitting_range"),RooFit.Color(1))
	...
	# qun
	Chi2Fit_data = xframe_data.chiSquare();
...
#qun 1120
	if self.channel=="el":
	   ...
	   #pure_wtagger_sf_el.Print();
           ...
           self.file_out_ttbar_control.write( "\n***************************************************" )
           self.file_out_ttbar_control.write( "\nQun Pure W-tagger SF of el %s     : %0.3f +/- %0.3f"%(label,pure_wtagger_sf_el, pure_wtagger_sf_el_err) )
	else:
	   ...
	   #pure_wtagger_sf_mu = self.workspace4fit_.var("eff_ttbar_TotalMC"+label+"_mu");
	   ...
	   #pure_wtagger_sf_mu.Print();
           print "Pure W-tagger SF of mu %s         : %0.3f +/- %0.3f"%(label,pure_wtagger_sf_mu, pure_wtagger_sf_mu_err)
           self.file_out_ttbar_control.write( "\n***************************************************" )
           self.file_out_ttbar_control.write( "\nQun Pure W-tagger SF of mu %s        : %0.3f +/- %0.3f"%(label,pure_wtagger_sf_mu, pure_wtagger_sf_mu_err) )
	print" qunqunqun:"
        # make the legend
	...
	# qun
	self.nPar_float_in_fitTo = rfresult_data.floatParsFinal().getSize();
	nBinX = xframe_data.GetNbinsX();
	ndof  = nBinX-self.nPar_float_in_fitTo;
	print xframe_data.chiSquare();
	print "#################### nPar=%s, chiSquare=%s/%s"%(self.nPar_float_in_fitTo ,xframe_data.chiSquare(self.nPar_float_in_fitTo)*ndof, ndof );
        ### write the result in the output
        self.file_out_ttbar_control.write("\n fit_mlvj_in_Mj_sideband: nPar=%s, chiSquare=%s/%s"%(self.nPar_float_in_fitTo, xframe_data.chiSquare( self.nPar_float_in_fitTo )*ndof, ndof ) );
	...
	#qun
        ...
        xframe_data.GetYaxis().SetRangeUser(1e-2,xframe_data.GetMaximum()*1.6);
        ...
        scale_number_TTbar_STop_VV_WJets_extremefail = (rdataset_TTbar_mj_extremefail.sumEntries()+rdataset_STop_mj_extremefail.sumEntries()+rdataset_VV_mj_extremefail.sumEntries() +rdataset_WJets_mj_extremefail.sumEntries() )/(rdataset_TTbar_mj_extremefail.sumEntries()+rdataset_STop_mj_extremefail.sumEntries()+rdataset_VV_mj_extremefail.sumEntries() +rdataset_WJets_mj_extremefail.sumEntries() )/( rdataset_data_mj_extremefail.sumEntries() )
        ...
        xframe_data_extremefail.GetYaxis().SetRangeUser(1e-2,xframe_data_extremefail.GetMaximum()*1.4);
...
#         simPdf_data.plotOn(xframe_data_fail,RooFit.Name("data fit invisi"),RooFit.Slice(category_p_f,"fail"),RooFit.ProjWData(RooArgSet(category_p_f),combData_p_f_data),RooFit.NormRange("controlsample_fitting_range"))
    ...
    ########## ---------------------------------------------------
    def get_mj_and_mlvj_dataset_TTbar_controlsample(self,in_file_name, label, jet_mass ="jet_mass_pruned"):#"massVhad"):# to get the shape of m_lvj jet_mass="jet_mass_pr" , "ttb_ca8_mass_pr"
        ...
        for i in range(treeIn.GetEntries()):
            ...
            if TMath.Abs(treeIn.CategoryID) > 2:
                ...
                if (self.categoryID == -4 or self.categoryID == 4):# LP
            ...
            if not label=="_data":
                ...
                #tmp_event_weight = abs(treeIn.weight)*tmp_lumi;
                #tmp_event_weight4fit = abs(treeIn.weight)*tmp_lumi/tmp_scale_to_lumi; #qun
    ...
    def banner4Plot(self, iswithpull=0):
      ...
      if iswithpull:
       if self.channel=="el":
        banner = TLatex(0.3,0.96,(" %.1f fb^{-1} at #sqrt{s} = 13TeV "%(self.GetLumi() )));
       ...
       if self.channel=="el":
        banner = TLatex(0.22,0.96,(" %.1f fb^{-1} at #sqrt{s} = 13TeV "%(self.GetLumi() )));
    ...
    def legend4Plot(self, plot, left=1, isFill=1, xoffset=0., yoffset=0., x_right_offset=0., y_upper_offset=0.):
        ...
        if left==-1:
            theLeg = TLegend(0.65+xoffset, 0.72+yoffset, 0.95+xoffset, 0.90+yoffset, "", "NDC");
            ...
            theLeg = TLegend(0.33+xoffset, 0.68+yoffset, 0.72+xoffset+x_right_offset, 0.95+yoffset+y_upper_offset, "", "NDC");
            #theLeg = TLegend(0.1+xoffset, 0.72+yoffset, 0.8+xoffset+x_right_offset, 0.95+yoffset+y_upper_offset, "", "NDC");
            theLeg.SetFillColor(0);
            theLeg.SetFillStyle(0);
	...
	theleg = TLegend(0.1+xoffset, 0.72+yoffset, 0.75+xoffset+x_right_offset, 0.95+yoffset+y_upper_offset, "", "NDC");
        theleg.SetBorderSize(0);
        theleg.SetLineColor(0);
        theleg.SetLineWidth(0);
        theleg.SetLineStyle(0);
        ...
        for obj in range(int(plot.numItems()) ):
            ...
            if not (((plot.getInvisible(objName)) and (not TString(objName).Contains("Uncertainty")) ) or TString(objName).Contains("invisi") or TString(objName).Contains("TLine") or objName==objName_before ):
                ...
                if TString(objName).Contains("Uncertainty") or TString(objName).Contains("sigma"):
                    ...
                    elif TString(objName).Data()=="VV" : theLeg.AddEntry(theObj, "VV","F");
        ...
        #theLeg.AddEntry(0, "","");
        theLeg.AddEntry(0, "W#rightarrow e/#mu #nu       %s"%(self.wtagger_label),"");
	theleg.AddEntry(0,"CMS Preliminary","");
        return theLeg;
        return theleg;#, theleg;
...
#        lumi2=self.add_CMS();
#        lumi2.Draw("same");
#        lumi3=self.add_Preliminary();
#        lumi3.Draw("same");
        lowX=0.21;
        lowY=0.78;
        lumi1  = TPaveText(lowX, lowY+0.06, lowX+0.15, lowY+0.16, "NDC");
        lumi1.SetTextFont(61);
        lumi1.SetTextSize(0.035);
	...
	lumi1.Draw("same");
        ...
        lowXa=0.21;
        lowYa=0.73;
        lumi2  = TPaveText(lowXa, lowYa+0.06, lowXa+0.15, lowYa+0.16, "NDC");
    ...
    def GetLumi(self):
        ...
        if self.channel=="el": return 2.198#1.26#0.732#0.57831#0.6269#0.2098 
        if self.channel=="mu": return 2.198#1.26#0.732#0.57831#0.6269#0.2098
    ...
    def fit_TTbar_controlsample(self, isherwig=0):
      ...
      if isherwig ==0:
        ...
        #self.fit_mj_single_MC(self.file_WJets0_mc,"_WJets0","GausChebychev","_TTbar_controlsample"); ## Erf*Exp pass sample
        self.fit_mj_single_MC(self.file_WJets0_mc,"_WJets0","ExpGaus","_TTbar_controlsample"); ## Erf*Exp pass sample
        #self.fit_mj_single_MC(self.file_WJets0_mc,"_WJets0","ErfExpGaus_sp","_TTbar_controlsample"); ## Erf*Exp pass sample
        ...
        if self.channel == "mu" :
              self.fit_mj_single_MC(self.file_VV_mc,"_VV","ExpGaus","_TTbar_controlsample"); ## Exp+Gaus pass
        ...
        self.ScaleFactor_forPureWJet_TTbar_controlsample(self.file_data);
          ...
          #self.fit_mj_single_MC(self.file_VV_mc,"_VV_herwig","2Gaus","_TTbar_controlsample");
          #self.fit_mj_single_MC(self.file_VV_mc,"_VV_herwig_failtau2tau1cut","2Gaus","_TTbar_controlsample");
